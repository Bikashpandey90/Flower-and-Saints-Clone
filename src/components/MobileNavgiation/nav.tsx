...
import CartSidebar from "../CartSideBar/cart"

interface MobileNavigationProps {
    onVisibilityChange?: (isVisible: boolean) => void
}

const MobileNavigation = ({ onVisibilityChange }: MobileNavigationProps) => {
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (currentScrollY < lastScrollY) {
                ...
                // Only hide after scrolling down past 100px to avoid hiding immediately
                setIsVisible(true)
            }
    ...
    return (
        <>
            <div
                className={`md:hidden rounded-t-2xl fixed bottom-0 left-0 right-0 bg-white border-t flex items-center justify-between px-4 py-2 z-10 transition-transform duration-300 ease-in-out ${isVisible ? "translate-y-0" : "translate-y-full"
                    }`}
            >
                <NavLink to="/" className="flex flex-col items-center">
                    <svg
                        className="icon icon-home icon-sm h-5 w-5"
                        viewBox="0 0 20 20"
                        stroke="currentColor"
                        fill="none"
                ...
                >
                    <svg
                        className="icon icon-hamburger icon-sm h-5 w-5"
                        viewBox="0 0 21 20"
                        stroke="currentColor"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    ...
                    <span className="text-[0.7rem] mt-1">Menu</span>
                </button>

                <button className="flex flex-col items-center" onClick={() => setIsSearchOpen(!isSearchOpen)}>
                    <svg
                        className="icon icon-search icon-sm h-5 w-5"
                        viewBox="0 0 20 20"
                        stroke="currentColor"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                    ...
                    <span className="text-[0.7rem] mt-1">Search</span>
                </button>

                <NavLink to="/shop" className="flex flex-col items-center">
                    <svg className="icon icon-grid icon-sm h-5 w-5" viewBox="0 0 21 20" stroke="currentColor" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <NavLink to="/account" className="flex flex-col items-center">
                    <svg
                        className="icon icon-account icon-sm h-5 w-5"
                        viewBox="0 0 20 20"
                        stroke="currentColor"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
